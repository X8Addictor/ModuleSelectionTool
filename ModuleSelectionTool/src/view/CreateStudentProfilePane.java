...
public class CreateStudentProfilePane extends GridPane {
	...
	//method to load the GUI when using load profile from menu bar
	public void setProfileDetails(StudentProfile sp) {
		
		cboCourses.getSelectionModel().select(sp.getStudentCourse());
		txtPnumber.setText(sp.getStudentPnumber());
		txtFirstName.setText(sp.getStudentName().getFirstName());
		txtSurname.setText(sp.getStudentName().getFamilyName());
		txtEmail.setText(sp.getStudentEmail());
		inputDate.setValue(sp.getSubmissionDate());
	}
